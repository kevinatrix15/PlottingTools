...
fitting_method = 'SkewNorm'; %'Gauss2';    % 'SkewNorm', 'Gauss2', 'Gauss1'

for t=236:1:tot_steps+1
    
    %----- 1D 2 term Ploting -----
    % Read in original interpolated temperature points
...
%     axis([-6e-4 6e-4 0 5000])
    ...
    %----- Read and plot 'conditioned' fitting points -----
    filename3 = ['Result_T', num2str(t-1), '_SurfTempXY_interpolated.csv'];
    MM = csvread(filename3, 1);
    ...
    x_end = find(MM(:,1) == xmax)           % Ending index of x' values

    xx3 = MM(1:x_end,1), xprof3 = MM(1:x_end,3);
    figure(2)
    subplot(1,2,1)
    plot(xx3, xprof3, 'gx', 'MarkerSize', 12), hold on
    
    yy3 = MM(x_end+1:end,2), yprof3 = MM(x_end+1:end,3);
    subplot(1,2,2)
    plot(yy3, yprof3, 'gx', 'MarkerSize', 12), hold on
...
%     axis([-6e-4 6e-4 0 5000])
    ...
    if (strcmp(fitting_method, 'Gauss2'))
        filename2 = ['Result_L1_T', num2str(t-1), '_Gauss2_1DParams.csv'];
    ...
    elseif (strcmp(fitting_method, 'SkewNorm'))
        filename2 = ['Result_L1_T', num2str(t-1), '_SkewNorm2_1DParams.csv'];
        N = csvread(filename2, 1);
        ...
        figure(2)
        subplot(1,2,1)
        SkewNorm1DPlotter(ax, bx, cx, ddx, alphx, -9e-4, 9e-4,'--k');
        hold off
        
        subplot(1,2,2)
        SkewNorm1DPlotter(ay, by, cy, ddy, alphy, -9e-4, 9e-4,'--k');
        hold off
%         pause
...
%     title(['t = ', num2str(t-1)]);
    ...
    for i = 1:Np
        ...
        for j = 1:Np
            ...
            elseif (strcmp(fitting_method, 'SkewNorm'))
                T_an(j,i) = SkewNorm2DDiscreteFunc(p, P0, theta, x, y);
            end
        end
    end
    cd(curr_dir)
